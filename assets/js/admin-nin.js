document.addEventListener('DOMContentLoaded', async () => {
    ...
    function renderOrders(ordersToRender) {
        ...
        ordersToRender.forEach(order => {
            ...
            tr.innerHTML = `
                <td><div class="service-col"><img src="../../assets/img/nimc-logo.png" style="width:24px;" alt="NIN"><span>${order.service_type || 'NIN Slip'}</span></div></td>
    ...
    async function loadAdminOrders() {
        const { data } = await window.db.rpc('admin_get_all_nin_orders');
        allOrders = data || [];
    ...
    saveBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            if (newStatus === 'COMPLETED' && fileInput.files.length > 0) {
                ...
                if (uploadError) throw new Error('Document upload failed. Ensure "nin_documents" bucket exists.');
                documentPath = filePath;
            }

            saveBtn.textContent = 'Finalizing...';
            const { error: rpcError } = await window.db.rpc('admin_update_nin_status', {
